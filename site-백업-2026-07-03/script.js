...
// ===== 고정 배너 (여러 개 가능) =====
// data/notice.json의 목록에서 표시 조건(active + 기간)에 맞는 배너를 전부 표시.
// 기간(start/end)을 비우면 상시 표시, end가 지나면 자동으로 사라짐.
async function loadNotice() {
  try {
    const res = await fetch("data/notice.json", { cache: "no-store" });
    if (!res.ok) return;
    const data = await res.json();
    const list = Array.isArray(data) ? data : [data]; // 예전 단일 형식도 허용
    ...
    `).join("");
    wrap.classList.toggle("notice-multi", visible.length > 1);
