...
첫 화면(hero) 배경은 `site\images\hero.jpg`(성도 단체사진 — "회복의 통로" 블로그 글의 잡지 스캔에서 크롭, 2026-07-03 적용). 사진 교체는 파일만 바꾸면 되고, 위치·어둡기 조절은 style.css의 `.hero background`에서.
...
- **배포 및 업데이트 계획 기록**: 로컬에서 수정한 내역을 바로 라이브에 반영하려면 Netlify 대시보드(Deploys)에 `site` 폴더를 통째로 다시 '드래그 앤 드롭'하여 수동 업데이트 가능. 향후 매일 새벽 5시 자동 업데이트(GitHub Actions)를 구성하기 위해 GitHub 저장소를 생성하고 Netlify와 연동할 예정임.
