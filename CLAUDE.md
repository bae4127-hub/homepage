...
- **네이버 블로그 예약 발행 시 RSS 날짜 문제**: 예약 발행 글은 RSS의 `pubDate`가 '예약된 게시일'이 아닌 '글을 작성한 날짜'로 나오는 경우가 있음. 이를 해결하기 위해 `fetch-feeds.mjs`와 `fetch-feeds-regex.ps1`에서 글 제목에 `OO년 O월 O일` 형식이 있으면 그 날짜를 `pubDate`보다 최우선으로 파싱해 사용하도록 수정함. (2026-07-17 겪음)

## ⚠️ 홈페이지 호스팅 및 배포 규칙 (GitHub Pages)

- **Netlify에서 GitHub Pages로 이전 완료 (2026-07-16)**: 기존 Netlify 무료 크레딧이 전부 소진되는 사태가 발생하여(매일 새벽 자동 푸시가 원인), 유지비 걱정이 없는 **GitHub Pages**로 홈페이지 호스팅을 완전히 이전했습니다.
- **배포 방식**: `.github/workflows/update.yml` 파일에서 새벽 5시(KST)마다 블로그/유튜브 피드를 수집한 직후, `actions/deploy-pages`를 통해 `site/` 폴더 전체를 GitHub Pages에 자동으로 배포하도록 통합 구축되어 있습니다. 
- **크레딧 제한 없음**: 이제 Netlify처럼 배포 횟수에 따른 크레딧 소진 위험이 없으므로, 로컬에서 작업한 뒤 자유롭게 `git push`를 진행해도 홈페이지가 무료로 무제한 갱신됩니다. 기존에 필요했던 '커밋/푸시 횟수 최소화' 규칙은 폐기되었습니다.
...
- **주간 신앙 가이드 연동 조건 완화 (2026-07-17)**: 글 제목뿐만 아니라 **블로그 카테고리명**에 키워드(말씀쇼츠, 공동체성경읽기 등)가 포함되어 있어도 자동으로 인식해 홈페이지에 연동하도록 `script.js` 수정.
- **SEO 소유권 확인 태그 삽입 (2026-07-17)**: 네이버 서치어드바이저 및 구글 서치 콘솔 사이트 소유권 확인용 메타 태그를 `index.html`에 삽입하고 배포 완료.
...
- **캐시 버전**: `style.css?v=10`, `script.js?v=4`으로 올림
## 관련 문서
